...
trait GBDB_AdvancedTrait {
    ...
    public static function getCachedData(string $database, string $table, bool $filter = false, mixed $where = '', mixed $is = '', int $ttl = 5): mixed {
        ...
        if (isset(self::$runtimeCache[$key]) && ($now - (int)self::$runtimeCache[$key]['ts']) <= max(1, $ttl)) {
            return self::$runtimeCache[$key]['data'];
        }

        $data = self::getData($database, $table, $filter, $where, $is);
        self::$runtimeCache[$key] = ['ts' => $now, 'data' => $data];
    ...
    /**
     * Migriert eine ältere GBDB-Struktur in eine neue GBDB-Struktur.
     *
     ...
     *
     * Wenn `$toPath` dem aktuellen `Vars::DB_PATH()` entspricht, wird über die normale
     * GBDB-API importiert. Dadurch funktionieren auch tokenisierte/verschlüsselte aktuelle
     * Installationen. Bei anderen Zielpfaden wird eine portable, unverschlüsselte GBDB-Struktur
     * mit Daten-, Meta- und Append-Dateien geschrieben.
     *
     * @param string $fromPath Quellpfad der alten GBDB/Projekt-Struktur.
     * @param string $toPath Zielpfad der neuen GBDB-Struktur.
     * @return array Migrationsbericht.
     */
    public static function migrateGBDB(string $fromPath, string $toPath): array {
        $source = self::resolveMigrationSourcePath($fromPath);
        $target = rtrim($toPath, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

        if ($source === null) {
            return ['ok' => false, 'message' => 'Quell-DB-Struktur nicht gefunden.', 'from' => $fromPath, 'to' => $toPath];
        }

        if (!is_dir($target) && !@mkdir($target, 0777, true) && !is_dir($target)) {
            return ['ok' => false, 'message' => 'Zielordner konnte nicht erstellt werden.', 'from' => $source, 'to' => $target];
        }

        $useLiveApi = realpath($target) !== false && realpath(Vars::DB_PATH()) !== false
            && rtrim((string)realpath($target), DIRECTORY_SEPARATOR) === rtrim((string)realpath(Vars::DB_PATH()), DIRECTORY_SEPARATOR);
        ...
        $dbDirs = self::migrationDatabaseDirs($source);

        foreach ($dbDirs as $dbName => $dbDir) {
            $dbName = Format::cleanString((string)$dbName);
            if ($dbName === '' || str_starts_with($dbName, '__')) continue;

            $report['databases']++;

            if ($useLiveApi && !in_array($dbName, self::listDBs(), true)) {
                self::createDatabase($dbName);
            }

            if (!$useLiveApi && !is_dir($target . $dbName)) {
                @mkdir($target . $dbName, 0777, true);
            }

            foreach (self::migrationTableFiles($dbDir) as $tableName => $file) {
                $tableName = Format::cleanString((string)$tableName);

                if ($tableName === '' || str_starts_with($tableName, '__')) continue;

                $tableData = self::readMigrationTableFile($file);

                if ($tableData === null) {
                    $report['skipped'][] = ['database' => $dbName, 'table' => $tableName, 'reason' => 'unreadable_or_not_json'];
                    continue;
                }

                $normalized = self::normalizeMigrationTableData($tableData);

                if (empty($normalized['header'])) {
                    $report['skipped'][] = ['database' => $dbName, 'table' => $tableName, 'reason' => 'no_columns'];
                    continue;
                }

                $cols = array_values(array_filter($normalized['header'], fn($c) => $c !== 'id'));
                $rows = $normalized['rows'];

                if ($useLiveApi) {
                    if (!in_array($tableName, self::listTables($dbName), true)) {
                        self::createTable($dbName, $tableName, $cols);
                    }

                    foreach ($rows as $row) {
                        $insert = $row;
                        unset($insert['id']);
                        $id = self::insertData($dbName, $tableName, $insert);

                        if ($id <= 0) {
                            $report['errors'][] = ['database' => $dbName, 'table' => $tableName, 'reason' => 'insert_failed'];
                        } else {
                            $report['rows']++;
                        }
                    }
                    ...
                    foreach ($cols as $col) $header[$col] = '-header-';
                    ...
                    foreach ($rows as $row) {
                        ...
                        foreach ($header as $col => $default) {
                            if ($col === 'id') continue;
                            $clean[$col] = array_key_exists($col, $row) ? $row[$col] : '';
                        }

                        $clean['id'] = isset($row['id']) ? (int)$row['id'] : ($lastId + 1);
                        $lastId = max($lastId, (int)$clean['id']);
                        $outRows[] = $clean;
                        $report['rows']++;
                    }

                    $targetFile = $target . $dbName . DIRECTORY_SEPARATOR . $tableName . '.json';
                    $metaFile = $target . $dbName . DIRECTORY_SEPARATOR . '__meta__' . $tableName . '.json';
                    $appendFile = $target . $dbName . DIRECTORY_SEPARATOR . '__append__' . $tableName . '.json';

                    self::migrationWriteJson($targetFile, $outRows);

                    self::migrationWriteJson($metaFile, [[
                        'version' => 2,
                        'last_id' => $lastId,
                        'rows' => count($rows),
                        'append_ops' => 0,
                        'indexes' => [],
                        'constraints' => [],
                        'migrations' => ['migrateGBDB'],
                        'checksum' => hash('sha256', json_encode($outRows, JSON_UNESCAPED_UNICODE) ?: ''),
                        'created_at' => time(),
                        'updated_at' => time()
                    ]]);
    ...
    /**
     * Findet den wahrscheinlichsten DB-Quellordner einer Migration.
     * @param string $fromPath Pfad.
     * @return string|null DB-Pfad oder null.
     */
    private static function resolveMigrationSourcePath(string $fromPath): ?string {
        $root = rtrim($fromPath, DIRECTORY_SEPARATOR);

        $candidates = [
            $root . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'DB' . DIRECTORY_SEPARATOR . 'GBDB',
            $root . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'DB',
            $root . DIRECTORY_SEPARATOR . 'DB',
            $root
        ];

        foreach ($candidates as $candidate) {
            if (!is_dir($candidate)) continue;
            if (!empty(self::migrationDatabaseDirs($candidate))) return $candidate;
        }
    ...
    /**
     * Listet Datenbankordner für eine Migration.
     * @param string $path DB-Basisordner.
     * @return array Datenbankname => Pfad.
     */
    private static function migrationDatabaseDirs(string $path): array {
        $out = [];
        $items = @scandir($path);
        ...
        foreach ($items as $item) {
            ...
            if (str_starts_with($item, '.') || str_starts_with($item, '__')) continue;
    ...
    /**
     * Listet potenzielle Tabellen-Dateien eines alten DB-Ordners.
     * @param string $dbDir Datenbankordner.
     * @return array Tabellenname => Datei.
     */
    private static function migrationTableFiles(string $dbDir): array {
        $out = [];
        $items = @scandir($dbDir);
        ...
        foreach ($items as $item) {
            if ($item === '.' || $item === '..') continue;
            if (str_starts_with($item, '__') || str_ends_with($item, '.lock')) continue;

            $full = rtrim($dbDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $item;

            if (!is_file($full)) continue;

            $ext = strtolower(pathinfo($item, PATHINFO_EXTENSION));

            if (!in_array($ext, ['json', 'db', 'txt', 'cry'], true)) continue;
    ...
    /**
     * Liest eine alte Tabellen-Datei mit Plain-JSON- und Crypt-Fallback.
     * @param string $file Datei.
     * @return array|null Tabelleninhalt.
     */
    private static function readMigrationTableFile(string $file): ?array {
    ...
    /**
     * Normalisiert alte GBDB-Tabellendaten auf Header + Rows.
     * @param array $tableData Rohdaten.
     * @return array Normalisierte Daten.
     */
    private static function normalizeMigrationTableData(array $tableData): array {
        $header = [];
        $rows = [];

        if (isset($tableData[0]) && is_array($tableData[0]) && isset($tableData[0]['id']) && (int)$tableData[0]['id'] === -1) {
            $header = array_keys($tableData[0]);
            ...
            foreach ($tableData as $row) {
                ...
                foreach (array_keys($row) as $key) {
                    if (!in_array($key, $header, true)) $header[] = $key;
                }

                $rows[] = $row;
            }

            if (!in_array('id', $header, true)) array_unshift($header, 'id');
        }

        $header = array_values(array_unique(array_map('strval', $header)));
        return ['header' => $header, 'rows' => $rows];
    }

    /**
     * Schreibt JSON atomar für externe Migrationsziele.
     * @param string $file Datei.
     * @param array $data Daten.
     * @return bool true bei Erfolg.
     */
    private static function migrationWriteJson(string $file, array $data): bool {
        $dir = dirname($file);

 ...
 * -> entspricht dem Ansatz aus gbdb_sys.php
 */
...
function json_pretty_flags(): int {
    // orientiert sich an Vars::jpretty() (DEV pretty, PROD compact)
    return Vars::jpretty();
}
 ...
 *
 * Datei-Name ist tokenisiert, Inhalt ist im enc-mode verschlüsselt (.db)
 */
...
/**
 * Meta/Append Datei-Namen (pro Tabelle!)
 * - plain: __meta__<table>.json, __append__<table>.json (werden bei plain nicht benötigt, aber wir ignorieren sie)
 * - enc: tokenisiert mit tblToken drin, damit es NICHT kollidiert
 *
 * Diese Names sind kompatibel zur Idee "meta/append pro tabelle" (wichtig!)
 */
...
function meta_filename_enc(string $tblToken, string $extEnc): string {
    // token enthält table-id, damit es pro table unique ist
...
function looks_like_meta_or_append_or_idx(string $file, string $extPlain, string $extEnc): bool {
    ...
    // encrypted tokenisierte meta/append/index files können nicht am prefix erkannt werden,
    // aber wir exportieren encrypted eh über table_index => nur echte Tabellen.
    return false;
}
 ...
 * Ignoriert meta/append/idx/locks.
 */
 ...
 *
 * Exportiert bewusst nur Tabellen, keine meta/append/index files.
 */
...
/**
 * Schätzt last_id und rows aus der Tabelle
 */
...
/**
 * Write encrypted schema (token dirs/files + indices + meta/append) into targetRoot
 */
function write_encrypted_schema(string $targetRoot, array $dump, string $extEnc): array {
    ...
    foreach ($dump as $dbPlain => $tables) {
        ...
        foreach ($tables as $tblPlain => $content) {
            ...
            // 3) Append pro Tabelle (leer)
            $appendFile = $dbDir . append_filename_enc($tblToken, $extEnc);
            // Append ist line-based; wir machen hier bewusst "leer" (nicht als JSON Array)
            // -> write_enc_db würde ein JSON bauen, das willst du nicht. Wir schreiben direkt.
            $empty = ""; // leerer Append
            $payload = Crypt::encode($empty); // bleibt decodierbar (ergibt "")
...
/**
 * Write plain structure into targetRoot (nur Tabellen, kein Meta/Append)
 */

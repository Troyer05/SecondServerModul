...
Sessionbasierter Tabellen-Cache für GBDB-Daten, damit häufig gelesene Tabellen nicht permanent neu geladen werden.
...
Danach steht `Cache` zur Verfügung.

## Typisches Beispiel

```php
$users = Cache::load("main", "users", []);
Cache::update("main", "users");
Cache::clear("users", "main");
```
...
|---|---|---|
|`load(string $db, string $table, mixed $cache)`|array|öffentliche Methode der Klasse|
|`update(string $db, string $table)`|void|öffentliche Methode der Klasse|
|`clear(string $table, string $db = null)`|void|öffentliche Methode der Klasse|
|`flush()`|void|öffentliche Methode der Klasse|
|`exists(string $table, string $db = null)`|bool|öffentliche Methode der Klasse|
|`all()`|array|öffentliche Methode der Klasse|

## Interne Hilfsmethoden

|Hilfsmethode|Sichtbarkeit|
|---|---|
|`ensureSession()`|protected|

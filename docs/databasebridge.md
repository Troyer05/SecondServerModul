# DatabaseBridge
## Zweck
`DatabaseBridge` abstrahiert GBDB und SQL, damit Anwendungscode nicht permanent zwischen beiden Backends unterscheiden muss.
## Datei und Einbindung
- Klasse: `DatabaseBridge`
- Datei: `assets/php/inc/gbdb_framework/core/database_bridge.php`
...
|---|---:|---|
| `get(string $db, string $table, bool $filter = false, string $where = "", string|int|float $is = "")` | `mixed` | Liest Daten aus der jeweiligen Quelle und gibt sie strukturiert zurück. |
| `insert(string $db, string $table, array $data)` | `mixed` | Fügt neue Daten ein. |
| `delete(string $db, string $table, string $where, string|int|float $is)` | `mixed` | Löscht Daten aus der angegebenen Quelle. |
| `update(string $db, string $table, string $where, string|int|float $is, array $data)` | `mixed` | Kapselt die Fachlogik für `update()` innerhalb dieser Klasse. |
...
$result = DatabaseBridge::get("example", "example", false, "", "");
var_dump($result);
```
...
- `private static isSQL() : bool` – Prüft ob SQL-Architektur aktiv ist
- `private static isGBDB() : bool` – Prüft ob GBDB (JSON) genutzt wird
- `private static ensureSQL() : void` – Stellt sicher, dass SQL verbunden ist

# Route
## Zweck
`Route` ist ein kleiner Router für PHP-Anwendungen. Er unterstützt HTTP-Methoden, dynamische Pfade, Middleware sowie eigene 404/405 Handler.
## Datei und Einbindung
- Klasse: `Route`
- Datei: `assets/php/inc/gbdb_framework/core/route.php`
...
|---|---:|---|
| `middleware(callable $mw)` | `self` | Mittels Middleware registrieren */ Verarbeitet die Funktion middleware. |
| `get(string $path, callable $callback)` | `void` | GET */ Liest Daten aus der angegebenen Quelle. |
| `post(string $path, callable $callback)` | `void` | POST */ Verarbeitet die Funktion post. |
| `put(string $path, callable $callback)` | `void` | PUT */ Verarbeitet die Funktion put. |
| `delete(string $path, callable $callback)` | `void` | DELETE */ Löscht Daten aus der angegebenen Quelle. |
| `patch(string $path, callable $callback)` | `void` | PATCH */ Verarbeitet die Funktion patch. |
| `notFound(callable $callback)` | `void` | Custom 404 */ Verarbeitet die Funktion not found. |
| `methodNotAllowed(callable $callback)` | `void` | Custom 405 */ Verarbeitet die Funktion method not allowed. |
| `dispatch()` | `void` | Dispatcher */ Verarbeitet die Funktion dispatch. |
...
- `private static addRoute(string $method, string $path, callable $callback) : void` – (Privat) Route registrieren */ Verarbeitet die Funktion add route.
- `private static makePattern(string $path) : string` – Parameter-Pattern konvertieren */ Verarbeitet die Funktion make pattern.

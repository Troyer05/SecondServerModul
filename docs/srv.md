# Srv
## Zweck
`Srv` läuft auf dem Backend-Server. Es verarbeitet Jobs, Module, Script-Ausführung und Auth-Aktionen, die über `SrvP` angefordert werden.
## Datei und Einbindung
- Klasse: `Srv`
- Datei: `assets/php/inc/Srv.php`
...
|---|---:|---|
| `enqueue(string $service, string $action, array $payload = [], array $ctx = [])` | `mixed` | Kapselt die Fachlogik für `enqueue()` innerhalb dieser Klasse. |
| `getJobs(array $ctx = [])` | `mixed` | Liest Daten aus der jeweiligen Quelle und gibt sie strukturiert zurück. |
| `getJob(int $id, array $ctx = [])` | `mixed` | Liest Daten aus der jeweiligen Quelle und gibt sie strukturiert zurück. |
| `runScript(string $path, array $params = [], array $ctx = [])` | `array` | Führt ein Script aus und gibt das Ergebnis zurück. |
| `auth(string $action, array $body)` | `array` | Verarbeitet Auth-Aktionen. |
| `runOne(int $id, array $ctx = [])` | `array` | Führt ein Script, eine Query oder einen Job aus. |
| `loadModule(string $service)` | `mixed` | Kapselt die Fachlogik für `loadModule()` innerhalb dieser Klasse. |
| `log(int $jobId, string $level, string $message, $extra = null)` | `mixed` | Kapselt die Fachlogik für `log()` innerhalb dieser Klasse. |
| `logs(int $jobId)` | `array` | Kapselt die Fachlogik für `logs()` innerhalb dieser Klasse. |
| `moduleLog(int $jobId, string $level, string $message, $extra = null)` | `mixed` | Kapselt die Fachlogik für `moduleLog()` innerhalb dieser Klasse. |

## Beispiele
```php
// läuft auf dem Backend-Server
...
- `private static driver(array $ctx = []) : string` – Liefert den aktiven DB-Treiber für optionale GBDBv2-Kontexte.
- `private static ensureTables(array $ctx = []) : void` – Stellt die SRV Tabellen sicher.
- `private static scriptPath(string $path) : string` – Normalisiert einen Script-Pfad relativ zum Projekt-Root des Backends.
...
## Zusatzhinweise
`runScript()` prüft die Datei auf der Backend-Seite. Das ist wichtig: Der Client übermittelt nur den relativen Pfad, ausgeführt wird ausschließlich, was auf Server 2 vorhanden und lesbar ist.

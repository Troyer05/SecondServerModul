...
SQL-Verbindungsklasse für MySQL/MariaDB-artige Nutzung.
...
Danach steht `SQL` zur Verfügung.
...
|---|---|---|
|`connect()`|bool|öffentliche Methode der Klasse|
|`select(string $table, string $select = "*", string $where = "", mixed $is = "",)`|array\|bool|öffentliche Methode der Klasse|
|`insert(string $table, array $data)`|bool|öffentliche Methode der Klasse|
|`update(string $table, array $data, string $where, mixed $is)`|bool|öffentliche Methode der Klasse|
|`delete(string $table, string $where, mixed $is)`|bool|öffentliche Methode der Klasse|

## Interne Hilfsmethoden

|Hilfsmethode|Sichtbarkeit|
|---|---|
|`run(string $query, array $params = [])`|private|

# SQL
## Zweck
`SQL` ist die optionale SQL-Brücke. Sie wird genutzt, wenn `DB_ARCH` auf SQL steht oder direkte SQL-Zugriffe nötig sind.
## Datei und Einbindung
- Klasse: `SQL`
- Datei: `assets/php/inc/gbdb_framework/core/sql.php`
...
|---|---:|---|
| `connect()` | `bool` | public static ?PDO $pdo = null; Stellt die Verbindung her (sicher, UTF-8, Fehlermodus) |
| `select(string $table, string $select = "*", string $where = "", mixed $is = "",)` | `array|bool` | SELECT |
| `insert(string $table, array $data)` | `bool` | INSERT |
| `update(string $table, array $data, string $where, mixed $is)` | `bool` | UPDATE |
| `delete(string $table, string $where, mixed $is)` | `bool` | DELETE |
...
- `private static run(string $query, array $params = []) : array|bool` – Interne: prepared statement ausführen

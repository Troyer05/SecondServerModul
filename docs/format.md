# Format
## Zweck
`Format` formatiert Datums-, Zeit- und Stringwerte für Eingabeformulare, HTML und kurze UI-Ausgaben.
## Datei und Einbindung
- Klasse: `Format`
- Datei: `assets/php/inc/gbdb_framework/core/format.php`
...
|---|---:|---|
| `dateForInput(mixed $date)` | `string` | Formatiert ein Datum für HTML <input type="date"> |
| `timeForInput(mixed $time)` | `string` | Formatiert eine Zeit für HTML <input type="time"> |
| `dateToView(mixed $date)` | `string` | Formatiert ein Datum für User-Anzeige (dd.mm.yyyy) |
| `shortString(string $string, int $maxLength = 14)` | `string` | Schneidet String sauber ab |
| `cleanString(string $string)` | `string` | Entfernt nicht-alphanumerische Zeichen (DE-kompatibel) Lässt äöüÄÖÜß zu |
| `newLineCode(string $string, bool $forHtml = true)` | `string` | Konvertiert zwischen Text-Input und HTML <br> |
...
$result = Format::dateForInput(null);
var_dump($result);
```
...
- `private static validDate(mixed $value) : bool` – Prüft ob ein Datum valide parsebar ist

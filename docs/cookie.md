# Cookie
## Zweck
`Cookie` kapselt Cookie-Erstellung, sichere Optionen, Lesen, Löschen, Vergleiche und Refresh-Logik.
## Datei und Einbindung
- Klasse: `Cookie`
- Datei: `assets/php/inc/gbdb_framework/core/cookies.php`
...
|---|---:|---|
| `set(string $name, string $value, int $expiration = self::DUR)` | `void` | Speichert einen Wert in der angegebenen Quelle. |
| `setSecure(string $name, string $value, int $expiration = self::DUR)` | `void` | Sicheres Cookie — nutzt dieselben Optionen, aber zwingt secure = true |
| `add(string $name, string $value)` | `void` | Kapselt die Fachlogik für `add()` innerhalb dieser Klasse. |
| `get(string $name)` | `mixed` | Liest Daten aus der angegebenen Quelle. |
| `delete(string $name)` | `void` | Löscht Daten aus der angegebenen Quelle. |
| `edit(string $name, string $value)` | `void` | Bearbeitet bestehende Daten. |
| `compare(string $name, string $value)` | `bool` | Kapselt die Fachlogik für `compare()` innerhalb dieser Klasse. |
| `refresh(int $thresholdSeconds = 3600)` | `void` | REFRESH entfernt jetzt Cookies NICHT und setzt sie NICHT neu. Das ist viel sinnvoller: → Nur erneuern, wenn Laufzeit kurz davor ist zu verfallen. |
| `init()` | `void` | Initialisiert die Klasse und legt benötigte Strukturen an. |
| `exists(string $name)` | `bool` | Prüft, ob ein Wert, eine Datei oder ein Datensatz existiert. |
...
$result = Cookie::set("example", "example", self::DUR);
var_dump($result);
```
...
- `protected static validateName(string $name) : string` – Validiere Cookie-Namen. Nur: a-z A-Z 0-9 _
- `protected static options(int $expiration, bool $secureOverride = null) : array` – Standardoptionen für Cookies
- `protected static send(string $name, string $value, int $expiration) : void` – Master-Setter für Cookies

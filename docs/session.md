# Session
## Zweck
`Session` ist Teil des greenbucket® GBDB/SecondServerModul-Frameworks. Die Klasse bündelt zusammengehörige Hilfs- und Framework-Funktionen, damit Anwendungen nicht direkt mit Dateien, Requests oder internen Tabellenstrukturen arbeiten müssen.

## Datei und Einbindung
- Klasse: `Session`
- Datei: `assets/php/inc/gbdb_framework/core/session.php`
...
|---|---:|---|
| `handler()` | `void` | Startet die Session sicher, falls sie nicht aktiv ist. Aktiviert sichere Cookie-Parameter. |
| `init()` | `void` | Initialisiert alle Framework-Session-Werte aus Vars::init_session() |
| `get(string $name)` | `mixed` | Holt den Wert einer Session-Variable |
| `set(string $name, mixed $value)` | `void` | Setzt oder überschreibt eine Session-Variable |
| `exists(string $name)` | `bool` | Prüft ob Session-Variable existiert |
| `delete(string $name)` | `void` | Löscht eine Session-Variable |
| `destroy()` | `void` | Zerstört die ganze Session (Logout) |
...
- `private static autoRegenerate() : void` – Regeneriert regelmäßig die Session-ID zum Schutz vor Hijacking.

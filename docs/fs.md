# FS
## Zweck
`FS` ist ein einfacheres Filesystem-Utility für Ordner, Dateien und rekursive Schreib-/Leseoperationen.
## Datei und Einbindung
- Klasse: `FS`
- Datei: `assets/php/inc/gbdb_framework/core/fs.php`
- Wird normalerweise über `assets/php/inc/gbdb_framework/gbdb.php` oder über `assets/php/inc/.config/_config.inc.php` geladen.

## Arbeitsweise
Die Klasse wird überwiegend objektorientiert bzw. gemischt genutzt. Öffentliche Methoden sind die stabile API für Projektcode. Private/protected Methoden sind interne Bausteine und sollten nicht direkt aus Anwendungen heraus verwendet werden.
...
|---|---:|---|
| `createFolder(string $pathAndName)` | `void` | Sichere Ordner-Erstellung */ Verarbeitet die Funktion create folder. |
| `write(string $file, mixed $data, bool $stream = false, bool $overwrite = false)` | `bool` | File schreiben + Stream-Option */ Verarbeitet die Funktion write. |
| `read(string $file)` | `mixed` | Datei lesen */ Verarbeitet die Funktion read. |
| `deleteDirectory(string $dir)` | `bool` | Sicheres rekursives Löschen eines Ordners */ Verarbeitet die Funktion delete directory. |
| `getFolderSize(string $path)` | `string` | Ordnergröße */ Verarbeitet die Funktion get folder size. |
| `deleteFiles(string $path)` | `bool` | Löscht alle Dateien innerhalb eines Ordners */ Verarbeitet die Funktion delete files. |
...
$result = FS::createFolder("example");
var_dump($result);
```

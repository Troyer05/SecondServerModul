...
ältere/kompakte Filesystem-Hilfsklasse für Ordner, Schreiben, Lesen und Löschen.
...
|---|---|---|
|`createFolder(string $pathAndName)`|void|öffentliche Methode der Klasse|
|`write(string $file, mixed $data, bool $stream = false, bool $overwrite = false)`|bool|öffentliche Methode der Klasse|
|`deleteDirectory(string $dir)`|bool|öffentliche Methode der Klasse|
|`getFolderSize(string $path)`|string|öffentliche Methode der Klasse|
|`deleteFiles(string $path)`|bool|öffentliche Methode der Klasse|
|`read(string $file)`|mixed|öffentliche Methode der Klasse|

# GetForm
## Zweck
`GetForm` verarbeitet Request- und Formularwerte in einer zentralen Klasse.
## Datei und Einbindung
- Klasse: `GetForm`
- Datei: `assets/php/inc/gbdb_framework/core/getForm.php`
...
|---|---:|---|
| `getDropdown(mixed $dropdown)` | `mixed` | Liest Daten aus der jeweiligen Quelle und gibt sie strukturiert zurück. |
| `upload(mixed $file, string $path = "./", string $useName = "")` | `bool` | Sichere Upload-Funktion (bis 2 MB, MIME-Check, sichere Namen) |
| `check_required_fields(mixed $post_data)` | `mixed` | Findet alle Pflichtfelder (_rf) |
| `createInput(string $name, string $type, mixed $form_data, string $placeholder = "", string $class = "", string $id = "")` | `string` | Erzeugt ein HTML Input-Feld |
| `checkPost()` | `bool` | Prüft ob POST-Request erfolgt ist |
...
$result = GetForm::getDropdown(null);
var_dump($result);
```

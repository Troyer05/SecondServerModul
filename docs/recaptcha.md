# ReCaptcha
## Zweck
`ReCaptcha` prüft Google reCAPTCHA-Antworten serverseitig mit den Keys aus `Vars`.
## Datei und Einbindung
- Klasse: `ReCaptcha`
- Datei: `assets/php/inc/gbdb_framework/core/recaptcha.php`
...
Benötigt `Vars::reCaptcha_website_key()` im Frontend und `Vars::reCaptcha_secret_key()` serverseitig.
...
|---|---:|---|
| `loadJsApi()` | `void` | Kapselt die Fachlogik für `loadJsApi()` innerhalb dieser Klasse. |
| `postName()` | `string` | Name der POST Variable |
| `checkBox(string $callbackJs = "")` | `string` | Gibt das HTML-Element für reCAPTCHA aus |
| `verify(?string $token)` | `bool` | Verifiziert das reCAPTCHA Token |

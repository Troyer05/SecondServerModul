# FileTool
## Zweck
`FileTool` ist ein robustes Dateisystem-Utility für Lesen, Schreiben, JSON, Backups, Ordnerkopien, Löschungen und Größenberechnung.
## Datei und Einbindung
- Klasse: `FileTool`
- Datei: `assets/php/inc/gbdb_framework/core/file_tool.php`
...
|---|---:|---|
| `exists(string $path)` | `bool` | Prüft, ob eine Datei existiert */ Verarbeitet die Funktion exists. |
| `read(string $path)` | `string` | Liest den Inhalt einer Datei (UTF-8, mit Locking) */ Verarbeitet die Funktion read. |
| `write(string $path, string $content)` | `bool` | Sicheres Schreiben (atomic write + locking) - schreibt zuerst in temp-file - dann rename() → atomarer System-Call |
| `readJson(string $path)` | `array` | Liest JSON und gibt Array zurück */ Verarbeitet die Funktion read json. |
| `writeJson(string $path, array $data)` | `bool` | Sicheres JSON-Schreiben (atomic) */ Verarbeitet die Funktion write json. |
| `delete(string $path)` | `bool` | Löscht Datei */ Löscht Daten aus der angegebenen Quelle. |
| `copyDir(string $src, string $dest)` | `void` | Kopiert ein ganzes Verzeichnis rekursiv */ Verarbeitet die Funktion copy dir. |
| `deleteOldFiles(string $dir, int $days)` | `void` | Löscht Dateien älter als X Tage */ Verarbeitet die Funktion delete old files. |
| `dirSize(string $dir)` | `float` | Berechnet Gesamtgröße eines Verzeichnisses in MB */ Verarbeitet die Funktion dir size. |
| `listFiles(string $dir, string $ext = '')` | `array` | Listet Dateien nach Endung */ Verarbeitet die Funktion list files. |
| `backupDir(string $src, string $dest)` | `bool` | Backup eines gesamten Verzeichnisses */ Verarbeitet die Funktion backup dir. |
| `listDirs(string $path)` | `array` | Alle Unterordner auflisten */ Verarbeitet die Funktion list dirs. |
...
$result = FileTool::exists("example");
var_dump($result);
```
...
- `private static ensureDir(string $dir) : void` – Verzeichnis sicher erstellen */ Verarbeitet die Funktion ensure dir.

...
Datei- und Verzeichnishelper für Lesen, Schreiben, JSON, Backups und Größenberechnung.
...
Danach steht `FileTool` zur Verfügung.

## Typisches Beispiel

```php
FileTool::writeJson("tmp/data.json", ["ok" => true]);
$data = FileTool::readJson("tmp/data.json");
```
...
|---|---|---|
|`exists(string $path)`|bool|öffentliche Methode der Klasse|
|`read(string $path)`|string|öffentliche Methode der Klasse|
|`write(string $path, string $content)`|bool|öffentliche Methode der Klasse|
|`readJson(string $path)`|array|öffentliche Methode der Klasse|
|`writeJson(string $path, array $data)`|bool|öffentliche Methode der Klasse|
|`delete(string $path)`|bool|öffentliche Methode der Klasse|
|`copyDir(string $src, string $dest)`|void|öffentliche Methode der Klasse|
|`deleteOldFiles(string $dir, int $days)`|void|öffentliche Methode der Klasse|
|`dirSize(string $dir)`|float|öffentliche Methode der Klasse|
|`listFiles(string $dir, string $ext = '')`|array|öffentliche Methode der Klasse|
|`backupDir(string $src, string $dest)`|bool|öffentliche Methode der Klasse|
|`listDirs(string $path)`|array|öffentliche Methode der Klasse|
